...
def main():
    ...
    for news in news_data:
        if "sentiment" in news and news["sentiment"]:
            sentiment_counts[news["sentiment"]] = (
                sentiment_counts.get(news["sentiment"], 0) + 1
            )
    ...
    with col3:
        st.metric(
            "Positive News",
            f"{positive_pct:.0f}%",
            f"{'+' if positive_pct > 50 else ''}{positive_pct - 50:.0f}%",
        )
    ...
    with st.sidebar:
        ...
        impact_filter = st.multiselect(
            "Impact Level", ["High", "Medium", "Low"], default=["High", "Medium", "Low"]
        )
    ...
    with col1:
        ...
        for news in news_data:
            ...
            if (
                symbol_filter
                and "symbols" in news
                and not any(symbol in news["symbols"] for symbol in symbol_filter)
            ):
                continue
            ...
            with st.container():
                ...
                with col_sentiment:
                    if "sentiment" in news:
                        ...
                        st.markdown(
                            f"<span style='color: {sentiment_color};'>●</span> {news['sentiment']}",
                ...
                with col_impact:
                    if "impact" in news:
                        ...
                        st.markdown(
                            f"<span style='color: {impact_color};'>●</span> {news['impact']}",
                            unsafe_allow_html=True,
                        )
                    else:
                        st.markdown("—")
                ...
                with col_symbols:
                    if "symbols" in news and news["symbols"]:
                        symbols_str = " ".join(
                            [f"`{symbol}`" for symbol in news["symbols"]]
                        )
    ...
    with col2:
        ...
        if filtered_news:
            ...
            if symbol_mentions:
                ...
                sorted_symbols = sorted(
                    symbol_mentions.items(), key=lambda x: x[1], reverse=True
                )
        ...
        if total_articles > 0:
            fear_greed = int(
                30 + (positive_pct * 0.7)
            )  # Scale 30-100 based on positive news
            if fear_greed >= 75:
        ...
        volatility_factor = (
            negative_pct / 100
        ) * 20  # Add up to 20 points for negative news
        volume_factor = min(
            5.0, total_articles / 20
        )  # Add up to 5 points for high news volume
        ...
        with col1:
            if st.button("🔔 Set News Alert", use_container_width=True):
                with st.form("news_alert_form"):
                    ...
                    alert_sentiment = st.selectbox(
                        "Alert on Sentiment", ["Positive", "Negative", "Both"]
                    )
                    alert_impact = st.selectbox(
                        "Minimum Impact Level", ["Low", "Medium", "High"]
                    )
                    email = st.text_input(
                        "Email Address (optional)", placeholder="your@email.com"
                    )

                    if st.form_submit_button("🔔 Activate Alert"):
                        st.success(
                            f"✅ News alert activated for {', '.join(alert_symbols)} with {alert_sentiment} sentiment!"
                        )
                        st.balloons()
        ...
        with col3:
            if st.button("🤖 AI Analysis", use_container_width=True):
                if has_groq_key:
                    ...
                    st.warning(
                        "⚠️ AI Analysis requires Groq API key. Configure it in the sidebar or main page."
                    )
            ...
            if st.button("📈 Market Impact", use_container_width=True):
                with st.spinner("Analyzing market impact correlation..."):
                    ...
                    if total_articles > 0:
                        ...
                        pos_impact = (
                            f"+{(positive_pct * 0.02):.1f}%"
                            if total_positive > 0
                            else "+0.0%"
                        )
                        neg_impact = (
                            f"-{(negative_pct * 0.015):.1f}%"
                            if total_negative > 0
                            else "-0.0%"
                        )
                        neutral_impact = (
                            f"+{(total_neutral / total_articles * 0.05):.1f}%"
                            if total_neutral > 0
                            else "+0.0%"
                        )
                        high_impact_est = (
                            f"+{(high_impact_count / total_articles * 1.5):.1f}%"
                    ...
                    st.info(
                        "📈 Impact estimates calculated from real news sentiment distribution and historical correlations."
                    )
        ...
        **API Integration:**
        ...
        with col1:
            ...
            new_freq = st.selectbox(
                ...
                index=["1 minute", "5 minutes", "15 minutes", "1 hour"].index(
                    st.session_state.news_update_freq
                ),
            )
        ...
        with col2:
            ...
            new_notifications = st.checkbox(
                "Enable Push Notifications", value=st.session_state.push_notifications
            )

...
def validate_ohlc_integrity(df: pd.DataFrame) -> dict:
    ...
    if bad_high: issues.append(f"{bad_high} rows: High < max(Open, Close)")
    if bad_low:  issues.append(f"{bad_low} rows: Low  > min(Open, Close)")
    if nonpos:   issues.append(f"{nonpos} rows: non-positive prices")
    if dup_idx:  issues.append(f"{dup_idx} duplicate dates")

    score = 100 - min(30, bad_high*0.05) - min(30, bad_low*0.05) - min(25, nonpos*0.05) - min(15, dup_idx*0.5)
...
def safe_detect_regimes(data: pd.DataFrame):
    try:
        ...
        out = data.copy()
        for src, col in [(vol,'Volatility_Regime'), (trend,'Trend_Regime'), (mkt,'Market_State_Regime')]:
            if isinstance(src, pd.DataFrame):
                c = [c for c in src.columns if 'regime' in c.lower()]
                if c: out[c[0].lower()] = src[c[0]]
...
def add_regime_spans(fig, df, regime_col='market_regime'):
    ...
    if regime_col not in df.columns: return
    ...
    for _, r in d.iterrows():
        if r[regime_col] != cur:
            if cur is not None: spans.append((start, prev_date, cur))
            cur = r[regime_col]; start = r['Date']
        prev_date = r['Date']
    if cur is not None: spans.append((start, prev_date, cur))
    for s,e,reg in spans:
        fig.add_vrect(x0=s, x1=e, fillcolor=colors.get(reg,'rgba(128,128,128,0.1)'),
...
def get_real_market_data(symbol, start_date, end_date):
    ...
    try:
        ...
        if data is not None and len(data) > 0:
            ...
            st.success(f"✅ Real data retrieved: {len(cleaned_data)} records | Data integrity: {integrity_score['integrity_score']:.1f}/100")
...
def backtest_strategy(data, strategy_type, params):
    ...
    if strategy_type in ['Mean Reversion', 'Regime-Aware Strategy']:
        ...
        upper, middle, lower = calculate_bollinger_bands(data, params.get('bb_period', 20))
        data['BB_upper'] = upper
        data['BB_lower'] = lower
    ...
    for i in range(len(data)):
        ...
        if strategy_type == 'SMA Crossover':
            if (row['SMA_short'] > row['SMA_long'] and 
                data.iloc[i-1]['SMA_short'] <= data.iloc[i-1]['SMA_long']):
                signal = 'BUY'
            ...
            if (row['Close'] < row['BB_lower'] and row['RSI'] < params.get('rsi_oversold', 30)):
                signal = 'BUY'
            elif (row['Close'] > row['BB_upper'] and row['RSI'] > params.get('rsi_overbought', 70)):
                signal = 'SELL'
...
def main():
    ...
    st.markdown("Test your trading strategies with production-grade data and regime detection")
    ...
    with st.sidebar:
        ...
        strategy_type = st.selectbox(
            "Strategy Type",
            ["SMA Crossover", "Mean Reversion", "Regime-Aware Strategy", "Buy and Hold"]
        )
        ...
        params['initial_capital'] = st.number_input("Initial Capital (₹)", min_value=1000, value=100000)
    ...
    if run_backtest:
        with st.spinner("Running backtest..."):
            ...
            if params.get('use_regime_detection', False) and 'market_regime' in backtest_results.columns:
                ...
                with col2:
                    ...
                    regime_timeline.add_trace(go.Scatter(
                    ...
                    for regime in regime_counts.index:
                        ...
                        if regime_mask.any():
                            ...
                            for i, (idx, row) in enumerate(regime_data.iterrows()):
                                if i == 0 or i == len(regime_data) - 1:  # Only show start and end for clarity
                                    regime_timeline.add_vrect(
                                        x0=idx, x1=idx,
                                        fillcolor=color_map.get(regime, 'rgba(128,128,128,0.1)'),
                                        opacity=0.3,
                                        layer="below",
                                        line_width=0,
                                    )
            ...
            with col2:
                ...
                monthly_returns = backtest_results.groupby('Month')['Portfolio_Value'].last().pct_change().dropna() * 100
            ...
            trades_df = backtest_results[backtest_results['Signal'].isin(['BUY', 'SELL'])].copy()
            if not trades_df.empty:
                trades_df = trades_df[['Date', 'Signal', 'Close', 'Portfolio_Value']].copy()
                    ...
                    with st.spinner("🧠 Analyzing strategy performance with AI..."):
                        try:
                            ...
                            if not config.get('groq_api_key'):
                                st.error("❌ GROQ API key not found in configuration. Please set GROQ_API_KEY in your .env file.")
                            ...
                            if params.get('use_regime_detection', False) and 'market_regime' in backtest_results.columns:
                                regime_distribution = backtest_results['market_regime'].value_counts().to_dict()
                                analysis_data['regime_distribution'] = regime_distribution
                            
                            # Get recent trades for analysis
                            recent_trades = trades_df.tail(10) if not trades_df.empty else pd.DataFrame()
                            if not recent_trades.empty:
                                analysis_data['recent_trades'] = recent_trades.to_dict('records')
                            ...
                            optimization_suggestions = ai_analyzer.analyze_data(strategy_specific_prompt)
                            ...
                            if metrics['max_drawdown'] < -10:  # More than 10% drawdown
                                ...
                                with st.expander("⚠️ Risk Management Focus", expanded=True):
                                    st.warning("High drawdown detected!")
                                    st.markdown(risk_analysis)
                            
                            # Market regime insights if available
                            if params.get('use_regime_detection', False) and 'regime_distribution' in analysis_data:
                                regime_prompt = f"""
                                ...
                                regime_analysis = ai_analyzer.analyze_data(regime_prompt)
                            ...
                            st.info("💡 Make sure your AI analyzer is properly configured with API keys.")

...
def simulate_prices(n_days: int = 240, seed: int = 42, start=1000.0):
    ...
    vol = pd.Series(
        rng.lognormal(12, 0.3, n_days).astype(int), index=dates
    )  # Reduced mean from 16 to 12
...
def universal_iterate_optimization(
    ...
    try:
        ...
        st.info(
            f"🎯 **Baseline Performance**: Sortino {baseline_objective:.3f}, Return {baseline_results.get('total_return', 0):.2f}%, DD {baseline_drawdown:.2f}%"
        )
        ...
        for iteration in range(max_iterations):
            ...
            if baseline_objective != 0:
                gain_pct = (
                    (trial_objective - baseline_objective)
            ...
            if baseline_objective <= 0:
                accepted = (trial_objective > baseline_objective) and (
                    dd_worsening <= max_dd_tolerance
                )
            else:
                accepted = (gain_pct >= min_gain_threshold) and (
                    dd_worsening <= max_dd_tolerance
                )
            ...
            if accepted:
                status_placeholder.success(
                    f"✅ **Improvement Found!** Sortino: {trial_objective:.3f} (Δ{iteration_data['objective_delta']:+.3f}), DD: {trial_drawdown:.2f}% (+{dd_worsening:.1f}pp)"
                )
                ...
                if baseline_objective <= 0:
                    status_placeholder.warning(
                        f"❌ **Trial Rejected** - Sortino: {trial_objective:.3f} (need > {baseline_objective:.3f}), DD worsening: {dd_worsening:.1f}pp (max {max_dd_tolerance})"
                    )
                else:
                    status_placeholder.warning(
                        f"❌ **Trial Rejected** - Gain: {gain_pct:.1f}% (need {min_gain_threshold}%), DD worsening: {dd_worsening:.1f}pp (max {max_dd_tolerance})"
                    )
...
def get_universal_ai_suggestion(
    ai_analyzer, strategy_config, current_params, current_perf, market_data
):
    """
...
with col2:
    ...
    if st.button("🤖 AI Optimize (Universal)", use_container_width=True):
        try:
            ...
            if not api_key:
                st.error(
                    "⚠️ GROQ_API_KEY not found in configuration. Please set it in .env file."
                )
            else:
                with st.spinner("🔄 Starting Universal AI Optimization..."):
                    ...
                    if strategy_type == "Momentum":
                        ...
                        current_params.update(
                            {"brk_period": brk_period, "vol_mult": vol_mult}
                        )
                    ...
                    if optimization_result and optimization_result.get(
                        ...
                        st.success(
                            "🎉 **Universal AI Optimization Completed with Improvements!**"
                        )
                        ...
                        with st.expander("📊 Optimization Results", expanded=True):
                            ...
                            with col_a:
                                ...
                                original_perf = run_actual_backtest(
                                    current_params, data
                                )
                                st.metric(
                                    "Sortino Ratio",
                                    f"{original_perf['sortino_ratio']:.3f}",
                                )
                                st.metric(
                                    "Total Return",
                                    f"{original_perf['total_return']:.2f}%",
                                )
                                st.metric(
                                    "Max Drawdown",
                                    f"{original_perf['max_drawdown']:.2f}%",
                                )
                                st.metric(
                                    "Win Rate", f"{original_perf['win_rate']:.1f}%"
                                )

                            with col_b:
                                st.markdown("**🚀 Optimized Strategy:**")
                                final_perf = optimization_result.get(
                                    "champion_perf", {}
                                )
                                improvement = (
                                    (
                                        (
                                            final_perf.get("sortino_ratio", 0)
                                            - original_perf["sortino_ratio"]
                                        )
                                        / original_perf["sortino_ratio"]
                                        * 100
                                    )
                                    if original_perf["sortino_ratio"] > 0
                                    else 0
                                )
                                st.metric(
                                    "Sortino Ratio",
                                    f"{final_perf.get('sortino_ratio', 0):.3f}",
                                    f"+{improvement:.1f}%",
                                )
                                st.metric(
                                    "Total Return",
                                    f"{final_perf.get('total_return', 0):.2f}%",
                                )
                                st.metric(
                                    "Max Drawdown",
                                    f"{final_perf.get('max_drawdown', 0):.2f}%",
                                )
                                st.metric(
                                    "Win Rate", f"{final_perf.get('win_rate', 0):.1f}%"
                                )

                        with st.expander("🔧 Optimized Parameters", expanded=True):
                            st.markdown("**🎯 Recommended Parameter Changes:**")

                            champion_params = optimization_result.get(
                                "champion_params", {}
                            )
                            ...
                            if strategy_type == "Momentum":
                                ...
                                with col1:
                                    old_period = current_params.get(
                                        "momentum_period", 20
                                    )
                                    new_period = champion_params.get(
                                        "momentum_period", old_period
                                    )
                                    change = (
                                        ((new_period - old_period) / old_period * 100)
                                        if old_period != 0
                                        else 0
                                    )
                                    st.metric(
                                ...
                                with col2:
                                    old_threshold = current_params.get(
                                        "momentum_threshold", 1.0
                                    )
                                    new_threshold = champion_params.get(
                                        "momentum_threshold", old_threshold
                                    )
                                    change = (
                                        (
                                ...
                                with col1:
                                    ...
                                    change = (
                                        ((new_rsi - old_rsi) / old_rsi * 100)
                                        if old_rsi != 0
                                        else 0
                                    )
                                    st.metric(
                                        "RSI Period", f"{new_rsi}", f"{change:+.1f}%"
                                    )

                                with col2:
                                    old_oversold = current_params.get("rsi_lo", 30)
                                    new_oversold = champion_params.get(
                                        "rsi_lo", old_oversold
                                    )
                                ...
                                with col3:
                                    ...
                                    new_overbought = champion_params.get(
                                        "rsi_hi", old_overbought
                                    )
                            ...
                            st.success(
                                "💡 **How to Apply**: Adjust the sliders in the sidebar to match these optimized values!"
                            )

                        with st.expander("📈 Optimization Journey", expanded=True):
                            iterations = optimization_result.get("iterations", [])
                            st.markdown(
                                f"**🎯 Total Iterations:** {optimization_result.get('total_iterations', 0)}"
                            )
                            ...
                            if iterations:
                                ...
                                for i, iter_data in enumerate(iterations):
                                    ...
                                    with st.expander(
                                        ...
                                        st.markdown(
                                            f"**Suggestions Applied:** {iter_data.get('suggestions_applied', 0)}"
                                        )
                                        st.markdown(
                                            f"**Objective Delta:** {iter_data.get('objective_delta', 0):+.3f}"
                                        )
                                        st.markdown(
                                            f"**Reasoning:** {iter_data.get('ai_reasoning', 'No reasoning provided')}"
                                        )
                                        if not iter_data.get("accepted", False):
                                            st.markdown(
                                                "**Why Rejected:** Performance improvement below threshold or drawdown increased too much"
                                            )

...
st.set_page_config(
    page_title="Dashboard", page_icon="📊", layout="wide", initial_sidebar_state="auto"
)
...
def main():
    ...
    with k3:
        if live_prices:
            avg_change = np.mean(
                [_to_float(v.get("pChange", 0)) for v in live_prices.values()]
            )
            st.metric(
                "Market Sentiment (proxy)",
                "—",
                f"{avg_change:+.2f}% avg across bellwethers",
            )
        else:

...
def mock_backtest_function(strategy_config):
    ...
    try:
        ...
        logger.info(f"Parameters - SMA Fast: {sma_fast}, SMA Slow: {sma_slow}, Risk: {risk_per_trade}")
        ...
        logger.info(f"Calculated multipliers - Trend: {trend_efficiency:.3f}, Lag: {lag_penalty:.3f}, Risk: {risk_scaling:.3f}")
        ...
        logger.info(f"Adjusted return: {adjusted_return:.5f}, volatility: {adjusted_volatility:.5f}")
        ...
        if max_signals > 0:
            signal_days = np.random.choice(len(portfolio_values), size=max_signals, replace=False)
        ...
        logger.info(f"Calculated metrics - Return: {total_return:.2f}%, Sharpe: {sharpe_ratio:.2f}, Drawdown: {max_drawdown:.2f}%")
...
def apply_changes(knobs: dict, changes: list, bounds: dict) -> dict:
    ...
    for i, ch in enumerate(changes[:3]):  # Limit to 3 changes max
        ...
        if p in bounds and isinstance(rng, list) and len(rng) == 2:
            ...
            logger.info(f"Applied change to {p}: {old_value} -> {new_value} (from range [{a:.4f}, {b:.4f}])")
        else:
            logger.warning(f"Skipped invalid change: param={p}, range={rng}, bounds_exist={p in bounds}")
    ...
    if "sma_fast" in new_knobs and "sma_slow" in new_knobs:
        ...
        if new_knobs["sma_fast"] >= new_knobs["sma_slow"]:
            logger.warning(f"Invariant violation: sma_fast ({original_fast}) >= sma_slow ({original_slow})")
            # Gently enforce gap
            new_knobs["sma_fast"] = max(bounds["sma_fast"][0], min(new_knobs["sma_slow"] - 1, bounds["sma_fast"][1]))
            logger.info(f"Enforced invariant: sma_fast adjusted to {new_knobs['sma_fast']}")
...
def iterate_optimization(
    ...
    try:
        ...
        def objective(p): return p.get("sortino_ratio", 0.0)
        ...
        for k, v in champion_cfg["knobs"].items():
            if isinstance(v, (int, float)):
                bounds[k] = [max(1, v*0.7), v*1.3] if v > 1 else [max(0.0001, v*0.7), v*1.3]
        ...
        for i in range(1, int(max_iters) + 1):
            ...
            try:
                ...
                for k, v in champion_cfg["knobs"].items():
                    if k in bounds:
                        knobs_for_analyzer[k] = bounds[k]  # Use the bounds we calculated
                    else:
            ...
            gain_pct = 0.0 if champion_obj == 0 else (obj_trial - champion_obj) / abs(champion_obj) * 100.0
            ...
            if (gain_pct >= float(min_gain_pct)) and (dd_worsen_pp <= float(max_dd_tol_pp)):
                # Promote
                ...
                decision = "rejected"
                logger.info(f"❌ Trial rejected - Gain: {gain_pct:.2f}% (need {min_gain_pct}%), DD worsening: {dd_worsen_pp:.2f} pp (max {max_dd_tol_pp})")
...
def display_optimization_results(result):
    ...
    with col4:
        promoted_count = len([it for it in result.get("iterations", []) if it.get("decision") == "promoted_to_champion"])
    ...
    if "iterations" in result and result["iterations"]:
        ...
        for it in result["iterations"]:
            ...
            history_data.append({
                ...
                "LLM Status": "✅ OK" if it.get("suggestion", {}).get("ok", True) else "❌ Error"
            })
...
def main():
    ...
    st.markdown("Powered by Groq AI for advanced market analysis and strategy optimization")
    ...
    tab1, tab2, tab3 = st.tabs(["💬 Chat Assistant", "🔄 Strategy Optimization", "⚙️ Configuration"])
    ...
    with tab1:
        ...
        with chat_container:
            ...
            if st.session_state.chat_history:
                ...
                with st.chat_message("assistant"):
                    st.write("👋 Hello! I'm your AI trading assistant. I can help you with:")
        ...
        user_input = st.chat_input("Ask me about trading strategies, market analysis, or risk management...")
            ...
            with col1:
                if st.button("📊 Market Analysis", use_container_width=True):
                    ...
                    with st.spinner("Analyzing..."):
                        try:
                            response = st.session_state.ai_analyzer.analyze_query(prompt)
                            st.session_state.chat_history.append({"role": "assistant", "content": response})
                        except Exception as e:
                            st.error(f"Error: {e}")
                    st.rerun()
    ...
    with tab2:
        ...
        st.markdown("Let AI iteratively improve your strategy parameters through automated backtesting")
        ...
        with col1:
            ...
            # Strategy setup
            strategy_name = st.text_input("Strategy Name", value="SMA Crossover Strategy")
            ...
            with col_param1:
                sma_fast = st.number_input("SMA Fast", min_value=5, max_value=50, value=10, step=1)
                
            with col_param2:
                sma_slow = st.number_input("SMA Slow", min_value=20, max_value=100, value=30, step=1)
                
            with col_param3:
                risk_per_trade = st.number_input("Risk per Trade", min_value=0.005, max_value=0.02, value=0.01, step=0.001, format="%.3f")
            ...
            with col_opt1:
                max_iterations = st.number_input("Max Iterations", min_value=3, max_value=10, value=5, step=1)
                
            with col_opt2:
                min_gain_pct = st.number_input("Min Gain % to Promote", min_value=5.0, max_value=20.0, value=10.0, step=1.0)
                
            with col_opt3:
                max_dd_tolerance = st.number_input("Max DD Tolerance (pp)", min_value=1.0, max_value=5.0, value=2.0, step=0.5)
            ...
            if abs(sma_fast - sma_slow) < 5:
                validation_warnings.append("⚠️ SMA gap too small - may generate < 10 trades")
            ...
            if st.button("🚀 Start AI Optimization", type="primary", use_container_width=True, disabled=button_disabled):
                logger.info("🚀 START AI OPTIMIZATION BUTTON CLICKED")
                logger.info(f"User parameters - SMA Fast: {sma_fast}, SMA Slow: {sma_slow}, Risk: {risk_per_trade}")
                logger.info(f"Optimization settings - Max iterations: {max_iterations}, Min gain: {min_gain_pct}%, Max DD tolerance: {max_dd_tolerance} pp")
                ...
                logger.info(f"Market data prepared for analyzer - Columns: {market_df.columns.tolist()}")
                ...
                try:
                    ...
                    if 'ai_analyzer' not in st.session_state:
                        ...
                        st.error("AI analyzer not initialized. Please check your API key.")
                    ...
                    logger.info(f"Optimization completed - Result keys: {result.keys()}")
                    ...
                    if result.get("total_iterations", 0) > 0:
                        promotions = len([it for it in result.get("iterations", []) if it.get("decision") == "promoted_to_champion"])
                        ...
                        "iterations": [
                            {
                                ...
                                "drawdown_pp": 1.5,
                                "config_after": {
                                    "knobs": {"sma_fast": sma_fast - 1, "sma_slow": sma_slow + 1, "risk_per_trade": risk_per_trade * 0.9}
                                },
                                "suggestion": {"ok": True, "changes": [{"param": "risk_per_trade", "new_range": [risk_per_trade * 0.85, risk_per_trade * 0.95]}]}
                            }
                        ],
                        "total_iterations": 1,
        ...
        with col2:
            ...
            **⚠️ Real constraints:**
            ...
            st.markdown("---")
            st.caption(f"**AI Model:** {getattr(st.session_state.ai_analyzer, 'model', 'llama-3.3-70b-versatile')}")

...
def fetch_live_data(symbols):
    ...
    try:
        ...
        if not live:
            return None
        clean = {sym: _normalize_live_dict({**v, "symbol": sym}) for sym, v in live.items()}
        return clean
    except Exception:
        return None
...
def generate_simulated_data(symbols):
    ...
    data = {}
    base_prices = {'RELIANCE': 2456.75, 'TCS': 3234.50, 'INFY': 1567.25, 'HDFCBANK': 1654.80, 'ICICIBANK': 1089.30}
    ...
    if order_type == 'BUY':
        ...
        if st.session_state.portfolio['cash'] >= total_cost:
            ...
            if symbol in st.session_state.portfolio['positions']:
                ...
                current_qty = st.session_state.portfolio['positions'][symbol]['quantity'] - quantity
                current_avg = st.session_state.portfolio['positions'][symbol]['avg_price']
                new_avg = ((current_qty * current_avg) + (quantity * market_price)) / (current_qty + quantity)
        ...
        if symbol in st.session_state.portfolio['positions'] and st.session_state.portfolio['positions'][symbol]['quantity'] >= quantity:
...
def main():
    ...
    if live_data is None:
        st.warning("⚠️ Live feed unavailable (market closed or connection). Using simulated ticks.")
        ...
        with order_col3:
            ...
            if st.button("Place Order", type="primary"):
                if live_data and selected_symbol in live_data:
                    ...
                    order = place_order(selected_symbol, quantity, order_type, limit_price)
                    ...
                    if order_mode == "MARKET":
                        ...
                        st.info("Limit order placed. Will execute when price reaches target.")

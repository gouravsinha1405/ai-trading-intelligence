...
def read_header(stream: IO[bytes]) -> Tuple[int, int]:
    ...
    assert version in (
        1, 2), "Unsupported git index version %i, only 1 and 2 are supported" % version
...
def aggressive_tree_merge(odb: "GitCmdObjectDB",
                          tree_shas: Sequence[bytes]) -> List[BaseIndexEntry]:
    R"""
    :return:

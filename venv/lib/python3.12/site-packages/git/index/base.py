...
def _named_temporary_file_for_subprocess(
        directory: PathLike) -> Generator[str, None, None]:
...
class IndexFile(LazyMixin, git_diff.Diffable, Serializable):
    ...
    def _serialize(
            self,
            stream: IO,
            ignore_extension_data: bool = False) -> "IndexFile":
    ...
    def from_tree(cls, repo: "Repo", *treeish: Treeish, **kwargs: Any) -> "IndexFile":
        ...
        if len(treeish) == 0 or len(treeish) > 3:
            raise ValueError(
                "Please specify between 1 and 3 treeish, got %i" %
                len(treeish))
    ...
    @unbare_repo
    def _iter_expand_paths(self: "IndexFile",
                           paths: Sequence[PathLike]) -> Iterator[PathLike]:
        ...
        for path in paths:
            ...
            # If the path is not already pointing to an existing file, resolve globs
            # if possible.
    ...
    @classmethod
    def entry_key(cls,
                  *entry: Union[BaseIndexEntry,
                                PathLike,
                                StageType]) -> Tuple[PathLike,
                                                     StageType]:
    ...
    def add(
        ...
        if entries:
            ...
            null_entries_indices = [i for i, e in enumerate(
                entries) if e.binsha == Object.NULL_BIN_SHA]
    ...
    def _flush_stdin_and_wait(
            cls,
            proc: "Popen[bytes]",
            ignore_stdout: bool = False) -> bytes:
        stdin_IO = proc.stdin
        if stdin_IO:
            stdin_IO.flush()
            stdin_IO.close()
    ...
    def checkout(
        ...
        **kwargs: Any,
        ...
        def handle_stderr(
                proc: "Popen[bytes]",
                iter_checked_out_files: Iterable[PathLike]) -> None:
            ...
            for line in stderr.splitlines():
                if not line.startswith(
                        "git checkout-index: ") and not line.startswith("git-checkout-index: "):
                    is_a_dir = " is a directory"
                    unlink_issue = "unable to unlink old '"
                    # created by entry.c:checkout_entry(...)

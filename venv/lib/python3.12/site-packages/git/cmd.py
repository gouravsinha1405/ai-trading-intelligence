...
def dict_to_slots_and__excluded_are_none(
        self: object, d: Mapping[str, Any], excluded: Sequence[str] = ()) -> None:
    for k, v in d.items():
        setattr(self, k, v)
    for k in excluded:
        setattr(self, k, None)
...
class _AutoInterrupt:
    ...
    def wait(self, stderr: Union[None, str, bytes] = b"") -> int:
        ...
        def read_all_from_possibly_closed_stream(
                stream: Union[IO[bytes], None]) -> bytes:
            if stream:
                try:
...
class Git(metaclass=_GitMeta):
    ...
    * https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessw
    ...
    def execute(
        ...
        **subprocess_kwargs: Any,
        ...
        stdout_sink = PIPE if with_stdout else getattr(
            subprocess, "DEVNULL", None) or open(
            os.devnull, "wb")
        if shell is None:
        ...
        try:
            if output_stream is None:
                ...
                if stdout_value.endswith(
                        newline) and strip_newline_in_stdout:  # type: ignore[arg-type]
    ...
    def transform_kwarg(
            ...
            name: str,
            value: Any,
            split_single_char_options: bool) -> List[str]:
        if len(name) == 1:
            if value is True:
                return ["-%s" % name]
    ...
    def _prepare_ref(self, ref: AnyStr) -> bytes:
        ...
        if isinstance(ref, bytes):
            # Assume 40 bytes hexsha - bin-to-ascii for some reason returns bytes, not
            # text.
    ...
    def __get_object_header(self, cmd: "Git.AutoInterrupt",
                            ref: AnyStr) -> Tuple[str, str, int]:
    ...
    def stream_object_data(
            self, ref: str) -> Tuple[str, str, int, "Git.CatFileContentStream"]:

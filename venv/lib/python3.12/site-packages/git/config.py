...
class MetaParserBuilder(abc.ABCMeta):  # noqa: B024
    ...
    def __new__(cls, name: str, bases: Tuple,
                clsdict: Dict[str, Any]) -> "MetaParserBuilder":
...
class GitConfigParser(cp.RawConfigParser, metaclass=MetaParserBuilder):
    ...
    def __init__(
        ...
        if file_or_files is not None:
            self._file_or_files: Union[PathLike, "BytesIO",
                                       Sequence[Union[PathLike, "BytesIO"]]] = file_or_files
        else:
    ...
    def _read(self, fp: Union[BufferedReader, IO[bytes]], fpname: str) -> None:
        ...
        while True:
            ...
            # An option line?
                ...
                if mo:
                    # We might just have handled the last line, which could contain a
                    # quotation we want to remove.
                    optname, vi, optval = mo.group("option", "vi", "value")
                    optname = self.optionxform(optname.rstrip())

                    if vi in (
                        "=",
                            ":") and ";" in optval and not optval.strip().startswith('"'):
                        pos = optval.find(";")
                    ...
                    if len(optval) < 2 or optval[0] != '"':
                        ...
                        # Opens quoting and does not close: appears to start multi-line
                        # quoting.
                        is_multi_line = True
                        optval = string_decode(optval[1:])
                    elif optval.find("\\", 1, -1) == -1 and optval.find('"', 1, -1) == -1:
                        # Opens and closes quoting. Single line, and all we need is
                        # quote removal.
                        optval = optval[1:-1]
                    # TODO: Handle other quoted content, especially well-formed
                    # backslash escapes.
            ...
            if self._has_includes():
                for _, include_path in self._included_paths():
                    ...
                    if not osp.isabs(include_path):
                        if not file_ok:
                            continue
                        # END ignore relative paths if we don't know the configuration
                        # file path
                        file_path = cast(PathLike, file_path)
                        assert osp.isabs(
                            file_path), "Need absolute paths to be sure our cycle checks will work"
    ...
    def write(self) -> None:
        ...
        if isinstance(self._file_or_files, (list, tuple)):
            raise AssertionError(
                "Cannot write back if there is not exactly a single file to write to, have %i files" % len(
                    self._file_or_files))

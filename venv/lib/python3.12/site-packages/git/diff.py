...
class Diff:
    ...
    def __str__(self) -> str:
        ...
        if self.diff:
            msg += "\n---"
            try:
                msg += self.diff.decode(defenc) if isinstance(self.diff,
                                                              bytes) else self.diff
    ...
    def rename_from(self) -> Optional[str]:
        return self.raw_rename_from.decode(
            defenc, "replace") if self.raw_rename_from else None

    @property
    def rename_to(self) -> Optional[str]:
        return self.raw_rename_to.decode(
            defenc, "replace") if self.raw_rename_to else None
    ...
    @classmethod
    def _pick_best_path(cls, path_match: bytes, rename_match: bytes,
                        path_fallback_match: bytes) -> Optional[bytes]:
        if path_match:
    ...
    @classmethod
    def _index_from_patch_format(cls,
                                 repo: "Repo",
                                 proc: Union["Popen",
                                             "Git.AutoInterrupt"]) -> DiffIndex["Diff"]:

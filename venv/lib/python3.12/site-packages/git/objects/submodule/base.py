...
class UpdateProgress(RemoteProgress):
    ...
    CLONE, FETCH, UPDWKTREE = [1 << x for x in range(
...
class Submodule(IndexObject, TraversableIterableObj):
    ...
    def _set_cache_(self, attr: str) -> None:
        if attr in ("path", "_url", "_branch_path"):
            ...
            self._branch_path = reader.get_value(
                self.k_head_option, git.Head.to_full_path(
                    self.k_head_default))
        elif attr == "_name":
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        # return self.path == other.path and self.url == other.url and
        # super().__eq__(other)
    ...
    def _write_git_file_and_module_config(
        ...
        with GitConfigParser(osp.join(module_abspath, "config"), read_only=False, merge_includes=False) as writer:
            writer.set_value("core", "worktree", to_native_path_linux(
        ...
        try:
            ...
            try:
                ...
                for i, remote in enumerate(rmts):
                    ...
                    progress.update(
                        op, i, len_rmts, prefix + "Fetching remote %s of submodule %r" %
                        (remote, self.name), )
                    # ===============================
                    if not dry_run:
                ...
                progress.update(BEGIN | CLONE, 0, 1, prefix +
            ...
            if mrepo is not None and mrepo.head.commit.binsha != binsha:
                ...
                if mrepo.head.commit.binsha != self.NULL_BIN_SHA:
                    ...
                    if may_reset and not force and mrepo.is_dirty(
        ...
        try:
            if configuration:
                try:
                    ...
                    raise InvalidGitRepositoryError(
                        "Submodule's entry at %r did not exist" %
                        (self.path)) from e
    ...
    def set_parent_commit(self,
                          commit: Union[Commit_ish,
                                        str,
                                        None],
                          check: bool = True) -> "Submodule":
    ...
    def module(self) -> "Repo":
        ...
        try:
            ...
            raise InvalidGitRepositoryError(
                "No valid repository at %s" %
                module_checkout_abspath) from e
        else:
            raise InvalidGitRepositoryError(

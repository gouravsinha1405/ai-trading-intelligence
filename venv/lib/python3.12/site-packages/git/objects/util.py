...
def get_object_type_by_name(
    ...
    if object_type_name == b"commit":
        ...
        raise ValueError(
            "Cannot handle unknown object type: %s" %
            object_type_name.decode())
...
def parse_date(string_date: Union[str, datetime]) -> Tuple[int, int]:
    ...
    try:
        ...
        raise ValueError(
            f"Unsupported date format or type: {string_date}, type={type(string_date)}") from e
...
class Traversable(Protocol):
    ...
    * :class:`Submodule <git.objects.submodule.base.Submodule>`
    ...
    def _list_traverse(
        ...
        if not as_edge:
            out: IterableList[Union["Commit", "Submodule",
                                    "Tree", "Blob"]] = IterableList(id)
    ...
    def _traverse(
        ...
        def addToStack(
            ...
            if branch_first:
                ...
                reviter = (TraverseNT(depth, lst[i], src_item)
...
class TraversableIterableObj(IterableObj, Traversable):
    ...
    def list_traverse(
            self: T_TIobj,
            *args: Any,
            **kwargs: Any) -> IterableList[T_TIobj]:

...
TraversedTreeTup = Union[Tuple[Union["Tree", None],
                               IndexObjUnion, Tuple["Submodule", "Submodule"]]]
...
class TreeModifier:
    ...
    def set_done(self) -> "TreeModifier":
        ...
        self._cache.sort(
            key=lambda x: (
                x[2] +
                "/") if x[1] == Tree.tree_id << 12 else x[2])
        return self

    # } END interface

    # { Mutators
    def add(self, sha: bytes, mode: int, name: str,
            force: bool = False) -> "TreeModifier":
...
class Tree(IndexObject, git_diff.Diffable, util.Traversable, util.Serializable):
    ...
    * You may likewise access by index, like ``blob = tree[0]``.
    ...
    def _iter_convert_to_object(
            self,
            iterable: Iterable[TreeCacheTup]) -> Iterator[IndexObjUnion]:
    ...
    def __reversed__(self) -> Iterator[IndexObjUnion]:
        return reversed(self._iter_convert_to_object(
            self._cache))  # type: ignore[call-overload]

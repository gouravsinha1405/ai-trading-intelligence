...
def py_where(program: str, path: Optional[PathLike] = None) -> List[str]:
    ...
    def is_exec(fpath: str) -> bool:
        return (
            osp.isfile(fpath) and os.access(
                fpath, os.X_OK) and (
...
def _is_cygwin_git(git_executable: str) -> bool:
    ...
    if is_cygwin is None:
        ...
        try:
            ...
            if not (
                pathlib.Path(uname_cmd).is_file() and os.access(
                    uname_cmd,
                    os.X_OK)):
                _logger.debug(
...
def finalize_process(proc: Union[subprocess.Popen,
                                 "Git.AutoInterrupt"],
                     **kwargs: Any) -> None:
...
def expand_path(p: Union[None, PathLike],
                expand_vars: bool = True) -> Optional[PathLike]:
...
class Actor:
    ...
    @classmethod
    def committer(cls,
                  config_reader: Union[None,
                                       "GitConfigParser",
                                       "SectionConstraint"] = None) -> "Actor":
        """
        :return:
...
class IterableList(List[T_IterableObj]):
    ...
    * :class:`RemoteReference <git.refs.remote.RemoteReference>`
    ...
    def __getitem__(self, index: Union[SupportsIndex,
                    int, slice, str]) -> T_IterableObj:
        if isinstance(index, int):
            ...
            try:
                ...
                raise IndexError(
                    "No item found with id %r" %
                    (self._prefix + index)) from e
        # END handle getattr
...
class IterableObj(Protocol):
    ...
    * :class:`Remote <git.remote.Remote>`
    ...
    @classmethod
    @abstractmethod
    def iter_items(cls, repo: "Repo", *args: Any, **
                   kwargs: Any) -> Iterator[T_IterableObj]:
    ...
    @classmethod
    def list_items(cls, repo: "Repo", *args: Any, **
                   kwargs: Any) -> IterableList[T_IterableObj]:

...
class Repo:
    ...
    * :attr:`git_dir` is the ``.git`` repository directory, which is always set.
    ...
    def __init__(
        ...
        try:
            common_dir = (Path(self.git_dir) /
                          "commondir").read_text().splitlines()[0].strip()
    ...
    def _get_config_path(self, config_level: Lit_config_levels,
                         git_dir: Optional[PathLike] = None) -> str:
        if git_dir is None:
    ...
    def config_writer(
            self,
            config_level: Lit_config_levels = "repository") -> GitConfigParser:
        """
        :return:
    ...
    def _get_untracked_files(self, *args: Any, **kwargs: Any) -> List[str]:
        ...
        for line in proc.stdout:
            ...
            if filename[0] == filename[-1] == '"':
                ...
                filename = filename.encode("ascii").decode(
                    "unicode_escape").encode("latin1").decode(defenc)
    ...
    def blame_incremental(self, rev: str | HEAD | None, file: str,
                          **kwargs: Any) -> Iterator["BlameEntry"]:
    ...
    def _clone(cls,
        ...
        #
        clone_path = Git.polish_url(
            path) if Git.is_cygwin() and "bare" in kwargs else path
        ...
        if not osp.isabs(path):
            path = osp.join(git._working_dir,
                            path) if git._working_dir is not None else path

...
def show_login_page():
    ...
    st.set_page_config(
        page_title="AI Trading Platform - Login", page_icon="🔐", layout="centered"
    )
...
def show_admin_panel():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["👥 Users", "➕ Add User", "🔐 Change Password", "📊 System Info"]
    )
                ...
                if user_to_modify:
                    ...
                    with col1a:
                        if st.button("🔄 Toggle Status"):
                            success, message = auth_manager.toggle_user_status(
                                user_to_modify
                            )
                            if success:
                                st.success(message)
    ...
    with tab2:
        ...
        with st.form("add_user_form"):
            ...
            new_password = st.text_input(
                "Password", type="password", placeholder="Enter password (min 6 chars)"
            )
            ...
            if st.form_submit_button("🔐 Change Password"):
                if new_password != confirm_password:
                    ...
                    success, message = auth_manager.change_password(
                        target_user, old_password, new_password
                    )
                    if success:
                        st.success(message)
                    else:
    ...
    with tab4:
        ...
        if recent_logins:
            for user in recent_logins[:5]:
                last_login = pd.to_datetime(user["last_login"]).strftime(
                    "%Y-%m-%d %H:%M"
                )

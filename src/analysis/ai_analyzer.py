...
class GroqAnalyzer:
    ...
    def _postvalidate(self, result: dict, knobs: dict, constraints: dict, invariants: List[str]) -> dict:
        """Enforce global constraints and validate areas/params post-LLM"""
        allowed_areas = {"gate", "entry", "exit", "sizing", "timing", "portfolio", "universe", "risk"}
        validated_changes = []
    ...
    def _make_request(self, prompt: str, system_prompt: str = None, json_mode: bool = False) -> str:
    ...
    def _compute_market_stats(self, price_data: pd.DataFrame) -> Dict:
        ...
        ret_stats = {
            ...
            "q": [round(q, 4) for q in returns.quantile([0.05, 0.25, 0.5, 0.75, 0.95]).tolist()]
        }
        ...
        if len(returns) > 20:
            ret_stats.update({
                ...
                "r2_80": round(self._calculate_trend_r2(price_data['Close'], 80), 3)
            })
    ...
    def _calculate_hurst(self, prices: pd.Series, max_lag: int = 100) -> float:
        ...
        try:
            ...
            valid_pairs = [(lag, t) for lag, t in zip(lags, tau) if t > 0 and np.isfinite(t)]
    ...
    def _extract_regime_diagnostics(self, data: pd.DataFrame, regime_data: pd.DataFrame = None) -> Dict:
        ...
        if not data.empty:
            ...
            if len(data) > 20:
                ...
                diagnostics["gate_vars"]["vol_proxy14_med"] = round(vol_proxy.median(), 3)
            ...
            if all(col in data.columns for col in ['High', 'Low']):
                ...
                if not atr_pct.empty:
                    ...
                    diagnostics["thresholds"]["atrp_lo"] = round(atr_pct.quantile(0.25), 4)
                    diagnostics["thresholds"]["atrp_hi"] = round(atr_pct.quantile(0.75), 4)
        ...
        if regime_data is not None and not regime_data.empty:
            if 'market_regime' in regime_data.columns:
                regime_counts = regime_data['market_regime'].value_counts(normalize=True)
                ...
                if 'returns' in regime_data.columns:
                    ...
                    for regime in regime_data['market_regime'].unique():
                        regime_returns = regime_data[regime_data['market_regime'] == regime]['returns']
    ...
    def _extract_news_signals(self, news_data: List[Dict]) -> List[Dict]:
        ...
        for item in (news_data or [])[:MAX_NEWS]:
            ...
            negative_words = ['fall', 'loss', 'decline', 'miss', 'downgrade', 'concern', 'weak']
            ...
            signal = {
                "ts": ts.isoformat(),
                "type": event_type,
                "entities": item.get('entities', [source]),  # Allow caller to pass tickers
    ...
    def optimize_strategy_structured(
        ...
        optimization_data = {
            "meta": {
                ...
                "train_range": strategy_config.get("train_range", "2019-01-01/2023-12-31"),
                "test_range": strategy_config.get("test_range", "2024-01-01/2025-08-15")
            },
            
            "market_stats": {
                "index": self._compute_market_stats(market_data)
            },
            
            "regime": self._extract_regime_diagnostics(market_data, regime_data),
            
            # Compact news early to keep prompts minimal
            "news_events": self._extract_news_signals(news_data)[:5] if news_data else [],
            
            "strategy": {
                "name": strategy_config.get("name", "unknown_strategy"),
                "pseudocode": strategy_config.get("description", "Strategy logic not provided"),
            ...
            "performance": {
                "headline": {
                    ...
                    "sortino": performance_metrics.get("sortino_ratio", performance_metrics.get("sharpe_ratio", 0) * 1.2),
                ...
                "fail_modes": performance_metrics.get("failure_modes", [
                    "Insufficient data for failure mode analysis"
                ])
            }
        }
...
Return ONLY JSON. No markdown, no prose."""
        ...
        try:
            ...
            return {"meta": self._get_telemetry_meta(), "ok": False, "error": f"JSON parse error: {e}", "raw_response": raw}
        
        # Validate response structure
        required_keys = {'ok', 'issues', 'changes', 'risks', 'test_plan'}
        if not required_keys.issubset(result.keys()):
            return {"meta": self._get_telemetry_meta(), "ok": False, "error": "Invalid response structure", "raw_response": result}
    ...
    def analyze_news_sentiment(self, news_data: List[Dict]) -> str:
        ...
        news_text = "\n".join([f"- {item.get('title', '')}: {item.get('summary', '')}" 
    ...
    def build_strategy_manifest(self, strategy_type: str, params: Dict) -> Dict:
        ...
        manifest = manifests.get(strategy_type, {
    ...
    def analyze_performance_slices(self, backtest_results: pd.DataFrame) -> Dict:
        ...
        total_return = (results_copy['Portfolio_Value'].iloc[-1] / results_copy['Portfolio_Value'].iloc[0] - 1)
        trading_days = len(results_copy)
        years = trading_days / 252
        
        headline = {
            "cagr": round((1 + total_return) ** (1/years) - 1 if years > 0 else 0, 3),
            "sharpe": round(returns.mean() / returns.std() * np.sqrt(252) if returns.std() > 0 else 0, 2),
            "max_dd": round(self._calculate_max_drawdown(results_copy['Portfolio_Value']), 3),
            "hit": round((returns > 0).mean(), 2),
            "avg_win": round(returns[returns > 0].mean() if len(returns[returns > 0]) > 0 else 0, 4),
            "avg_loss": round(returns[returns < 0].mean() if len(returns[returns < 0]) > 0 else 0, 4),
        ...
        if 'Date' in results_copy.columns or results_copy.index.name == 'Date':
            ...
            for day in results_copy['Weekday'].unique():
                ...
                if len(day_returns) > 0:
                    ...
                    weekday_performance[day] = {"pf": round(min(profit_factor, 999.9), 2)}
        ...
        if len(returns) > 30:
            ...
            if len(low_vol_returns) > 0 and low_vol_returns.std() > 0:
                vol_terciles["low"] = {"sharpe": round(low_vol_returns.mean() / low_vol_returns.std() * np.sqrt(252), 2)}
            if len(high_vol_returns) > 0 and high_vol_returns.std() > 0:
                vol_terciles["high"] = {"sharpe": round(high_vol_returns.mean() / high_vol_returns.std() * np.sqrt(252), 2)}
        ...
        if worst_dd_idx is not None:
            fail_modes.append(f"Worst drawdown: {round(dd_series.min()*100, 1)}% around period {worst_dd_idx}")
    ...
    def _apply_llm_changes(self, current_knobs: dict, changes: list) -> dict:
        ...
        for ch in changes:
            ...
            if p in new_knobs and isinstance(new_knobs[p], (list, tuple)) and lo is not None and hi is not None:
                # If knob stores an allowed range, keep it. If it stores a point value, set to midpoint.
                if isinstance(new_knobs[p], (int, float)):
                    new_knobs[p] = round((lo + hi) / 2, 6)
                else:
                    # Keep allowed range but bias a suggested "current" value you store elsewhere if needed.
                    pass
    ...
    def iterate_improvement(
        self,
        run_backtest_fn,              # callable: (strategy_config) -> (equity_df, perf_metrics_dict)
        strategy_config: dict,
        market_data: pd.DataFrame,
        regime_data: Optional[pd.DataFrame] = None,
        news_data: Optional[list] = None,
        max_iters: int = 6,
        min_oos_gain_pct: float = 10.0,   # promotion threshold on objective (e.g., Sortino)
        ...
        champion_obj = champion_perf.get("sortino_ratio", champion_perf.get("sharpe_ratio", 0.0))
        ...
        for it in range(1, max_iters + 1):
            ...
            new_cfg["knobs"] = self._apply_llm_changes(current_cfg.get("knobs", {}), suggestion["changes"])
            ...
            if promote:
                champion_cfg, champion_eq, champion_perf, champion_slices = new_cfg, ch_eq, ch_perf, ch_slices
            ...
            if it >= 2 and history[-1]["decision"] != "promoted_to_champion" and history[-2]["decision"] != "promoted_to_champion":
                break

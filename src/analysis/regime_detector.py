...
class RegimeDetector:
    ...
    def detect_volatility_regimes(
        ...
        try:
            ...
            volatility = returns.rolling(window=window).std() * np.sqrt(
                252
            )  # Annualized
            ...
            self.logger.info(
                f"Detected {n_regimes} volatility regimes using {'walk-forward' if use_walkforward else 'quantile'} method"
            )
            return result_df
    ...
    def detect_trend_regimes(
        ...
        try:
            ...
            self.logger.info(
                "Trend regimes detected successfully with EMA slope method"
            )
            return result_df
    ...
    def detect_market_state_regimes(
        self,
        data: pd.DataFrame,
        window: int = 30,
        use_gmm: bool = True,
        ...
        try:
            ...
            if len(features_df) < max(window * 2, 100):  # Minimum data requirement
                self.logger.warning(
                    "Insufficient data for market state regime detection"
                )
                return data
            ...
            result_df.loc[common_index, "Market_State_Regime"] = regime_labels.loc[
                common_index
            ]
            ...
            if regime_probs is not None and len(regime_probs) > 0:
                ...
                for i, regime_name in enumerate(
                    ["Bull_Market", "Bear_Market", "High_Volatility", "Sideways_Market"]
                ):
                    if i < regime_probs.shape[1]:
                        prob_series = pd.Series(dtype=float, index=data.index)
                        if len(prob_index) <= len(regime_probs):
                            prob_series.loc[prob_index] = regime_probs[
                                : len(prob_index), i
                            ]
                        result_df[f"{regime_name}_Prob"] = prob_series

            self.logger.info(
                "Market state regimes detected successfully with walk-forward approach"
            )
            return result_df
            ...
            for date, regime in regime_series.items():
                if regime != current_regime and current_regime is not None:
                    change_event = {
                        ...
                        "to_regime": regime,
                        "change_type": self._classify_regime_change(
        ...
        try:
            if use_full_features:
                ...
                probabilities = self._calculate_feature_based_probabilities(
                    current_features,
                    historical_features,
                    historical_regimes[regime_column],
                )
            else:
                ...
                for regime in historical_regimes[regime_column].unique():
                    ...
                    regime_data = historical_regimes[
                        historical_regimes[regime_column] == regime
                    ]
                ...
                for regime, chars in regime_chars.items():
                    ...
                    # Inverse distance as similarity (add small constant to avoid
                    # division by zero)
    ...
    def _kmeans_vol_regimes_walkforward(
        self,
        ...
        try:
            pipe = Pipeline(
                [
                    ...
                    (
                        "kmeans",
                        KMeans(n_clusters=n_regimes, random_state=42, n_init="auto"),
                    ),
                ]
            )
            ...
            for start in range(
                0, len(idx) - (train_window + test_window) + 1, test_window
            ):
    ...
    def _rolling_quantile_regimes(
        self, vol_series: pd.Series, n_regimes: int = 3, window: int = 252
    ) -> pd.Series:
    ...
    def _rolling_r2_logprice(
        self, price_series: pd.Series, window: int = 80
    ) -> pd.Series:
    ...
    def _gmm_market_regimes_walkforward(
        self,
        features_df: pd.DataFrame,
        n_components: int = 4,
        train_window: int = 750,
        ...
        try:
            ...
            for start in range(
                ...
                mapped_labels = [
                    regime_names.get(label, f"Regime_{label}") for label in test_labels
                ]
                labels.loc[test_idx] = mapped_labels
    ...
    def _kmeans_market_regimes_walkforward(
        self,
        features_df: pd.DataFrame,
        n_clusters: int = 4,
        train_window: int = 750,
    ...
    def _assign_regime_names(
        self, features_df: pd.DataFrame, cluster_labels: np.ndarray
    ) -> Dict:
        ...
        try:
            ...
            for i in range(n_clusters):
                ...
                if (
                    avg_returns > return_pos_threshold
                    and avg_volatility < vol_threshold
                ):
                    regime_names[i] = "Bull_Market"
                elif (
                    avg_returns < return_neg_threshold
                    and avg_volatility < vol_threshold
                ):
    ...
    def _calculate_feature_based_probabilities(
        self,

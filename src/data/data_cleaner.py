...
class DataCleaner:
    ...
    def clean_ohlcv_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if not set(req).issubset(x.columns):
                self.logger.error(
                    f"Missing required columns: {set(req) - set(x.columns)}")
                return df
            ...
            if isinstance(x.index, pd.DatetimeIndex) and x.index.tz is None:
                try:
                    ...
                    if x.index.duplicated().any():
                        self.logger.warning(
                            "Found remaining duplicates, cleaning again")
                        x = x[~x.index.duplicated(keep='last')].copy()
    ...
    def detect_outliers_robust(
            ...
            df: pd.DataFrame,
            window: int = 100,
            threshold: float = 8.0) -> pd.DataFrame:
        """
        ...
        try:
            ...
            for col in available_cols:
                ...
                roll_med = cleaned_df[col].rolling(
                    window, min_periods=window // 2).median()
                roll_mad = cleaned_df[col].rolling(
                    window,
                    min_periods=window //
                    2).apply(
                    lambda x: mad(
                ...
                if outliers_found > 0:
                    self.logger.warning(
                        f"Found {outliers_found} outliers in {col} (review manually before dropping)")
                    # Log some examples for manual review
                    outlier_samples = cleaned_df[outlier_mask][col].head(3)
                    self.logger.info(
                        f"Example outliers in {col}: {outlier_samples.tolist()}")

                # Remove extreme outliers (conservative threshold)
                cleaned_df = cleaned_df[~outlier_mask]

            self.logger.info(
                f"Robust outlier detection: {len(df)} -> {len(cleaned_df)} records")
            return cleaned_df
    ...
    def handle_missing_data(
            self,
            df: pd.DataFrame,
            method: str = 'keep_nan') -> pd.DataFrame:
        """
        ...
        try:
            ...
            if method == 'keep_nan':
                # Keep NaN for OHLC (don't fabricate prices), only fill non-price
                # columns
    ...
    def detect_data_gaps(self, df: pd.DataFrame,
                         sessions: Optional[pd.DatetimeIndex] = None) -> List[Dict]:
        """
        ...
        try:
            ...
            if sessions is None:
                # Fallback: business days only - better to pass NSE sessions including
                # holidays
                ...
                self.logger.info(
                    "Using business days as trading sessions (consider providing NSE calendar)")
            ...
            missing_df['group'] = (missing_df['date'].diff() >
                                   pd.Timedelta('1D')).cumsum()
            ...
            self.logger.info(
                f"Found {len(gaps)} trading session gaps (excluding weekends/holidays)")
            return gaps
    ...
    def validate_data_quality(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            duplicate_score = max(
                0, 100 - (quality_metrics['duplicate_records'] / len(df)) * 100)
            outlier_score = max(
                0, 100 - (quality_metrics['outlier_records'] / len(df)) * 100)

            quality_metrics['data_quality_score'] = (
                missing_score + duplicate_score + outlier_score) / 3

            self.logger.info(
                f"Data quality score: {quality_metrics['data_quality_score']:.2f}")
    ...
    def standardize_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'Volume' in standardized_df.columns:
                standardized_df['Volume'] = pd.to_numeric(
                    standardized_df['Volume'], errors='coerce')
    ...
    def resample_data(self, df: pd.DataFrame, target_freq: str = '5T') -> pd.DataFrame:
        ...
        try:
            ...
            available_rules = {col: rule for col,
                               rule in agg_rules.items() if col in df.columns}
            ...
            other_cols = [
                col for col in df.columns if col not in available_rules.keys()]
            if other_cols:
            ...
            if len(resampled_df) > 0:
                ...
                available_ohlc = [
                    col for col in ohlc_cols if col in resampled_df.columns]
            ...
            self.logger.info(
                f"Data resampled to {target_freq}: {len(df)} -> {len(resampled_df)} bars")
            return resampled_df
    ...
    def get_nse_trading_calendar(
        ...
        try:
            ...
            holiday_dates = pd.to_datetime(
                nse_holidays_2024_2025).tz_localize('Asia/Kolkata')

            # Remove holidays from business days
            trading_sessions = business_days.difference(holiday_dates)

            self.logger.info(
                f"Generated NSE trading calendar: {len(trading_sessions)} sessions")
    ...
    def adjust_for_corporate_actions(
        ...
        try:
            ...
            self.logger.warning(
                "Using placeholder corporate actions - implement real data source")
        ...
        try:
            ...
            for col in available_ohlc:
                try:
                    ...
                    self.logger.warning(
                        f"Negative price check failed for {col}: {neg_e}")

            # Volume checks
            if 'Volume' in df.columns:
                try:
                    volume_mask = (df['Volume'] == 0).fillna(False)
                    validation_results['zero_volume_days'] = sum(
                        volume_mask)  # Simple sum() function
            ...
            if 'Close' in df.columns and len(df) > 1:
                try:
                    ...
                    validation_results['suspicious_gaps'] = sum(
                        gap_mask)  # Simple sum() function
            ...
            self.logger.info(
                f"OHLC integrity score: {validation_results['integrity_score']}")
            return validation_results

    ...
    def get_index_data(self, index: str, from_date: datetime, to_date: datetime) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                self.logger.warning(f"No data found for {index} from {from_date} to {to_date}")
                return None
    ...
    def get_stocks_data(self, symbols: List[str], start_date: str, end_date: str, 
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                if i > 0:
                    delay = self.rate_limit + random.uniform(0, 0.5)  # Add jitter  # nosec B311
                    time.sleep(delay)
                ...
                if df is not None and not df.empty:
                    results[symbol] = df
                    self.logger.info(f"Successfully fetched data for {symbol}: {len(df)} records")
                else:
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if hasattr(self, 'data_cleaner') and self.data_cleaner:
                ...
                available_cols = [col for col in essential_cols if col in cleaned_df.columns]
